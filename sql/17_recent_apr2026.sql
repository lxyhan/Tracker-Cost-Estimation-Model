-- Most-recent HTTP Archive crawl (April 2026) — for deployment-realistic
-- Claim B validation. Trains a model that is ~1 month old at the time of
-- the in-page Firefox crawl (run May 2026), matching the operational
-- staleness assumption of the quarterly retraining cadence in §6.
--
-- The June 2024 training data and all temporal-curve holdouts (sql/07-10)
-- remain the paper's primary measurement; this query produces an additional
-- model used only for Claim B's deployment validation.
--
-- Output: data/raw/17_recent_apr2026/per_request_apr2026.csv (~3-4M rows)
...
WHERE req.date = '2026-04-01'  -- confirmed via INFORMATION_SCHEMA: 5.4B rows
  AND req.client = 'mobile'
  AND req.is_root_page = TRUE
  AND NET.HOST(req.url) IN (
    SELECT domain
    FROM `httparchive.almanac.third_parties`
    WHERE date = '2025-07-01'  -- most recent third-parties classification (confirmed)
